...
window.Vaadin['_vaadintheme_test-schema_globalCss'] = window.Vaadin['_vaadintheme_test-schema_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_test-schema_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_test-schema_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_test-schema_globalCss'].push(target);
  }
  if (!document['_vaadintheme_test-schema_componentCss']) {
    
    document['_vaadintheme_test-schema_componentCss'] = true;
  }
